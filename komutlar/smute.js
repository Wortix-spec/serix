...
module.exports.run = async (bot, message, args) => {
  ...
  if (!mutekisi)
    return message.reply(
      `:x: Lütfen bir kullanıcı etiketleyiniz! \nDoğru Kullanım; \`${prefix}smute <@kullanıcı> <1sn/1dk/1sa/1g>\``
    );
  if (mutekisi.hasPermission("MANAGE_MESSAGES"))
    return message.reply(
      `:x: Yetkili bir kişiyi muteleyemem! \nDoğru Kullanım; \`${prefix}smute <@kullanıcı> <1sn/1dk/1sa/1g>\``
    );
  ...
  setTimeout(function() {
    ...
    message.channel.send(
      `<@${mutekisi.id}> Kullanıcı Muteleme Süresi Sona Erdi!`
    );
  }, ms(mutezaman));
};
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ["tempmute", "sürelimute", "smute"],
  permLevel: 0
};
...
exports.help = {
  ...
  usage: "tempmute <@kullanıcı> <1sn/1dk/1sa/1g>"
};
//Omen Bot
