...
exports.run = async (bot, message, args) => {
  ...
  if (!message.member.hasPermission("BAN_MEMBERS"))
    return message.channel.send(
      "Bu komudu kullanabilmek için `Üyeleri Yasakla` yetkisine sahip olmanız gerek."
    );
  let reason = args.slice(1).join(" ");

  if (!args[0])
    return message.channel.send(
      "Yasaklamak istediğiniz kullanıcıyı etiketleyiniz."
    );
  let user =
  ...
  if (!user)
    return message.channel.send(
      `Etiketlediğin kullanıcıyı sunucuda bulamadım.`
    );
  ...
  message.channel.send(
    `${user.tag}, Bu kullanıcıyı sunucudan yasaklamak için **evet** Yasaklakmamak için ise **hayır** Yazınız..`
  );
  let uwu = false;
  while (!uwu) {
  ...
  if (uwu) {
    try {
      ...
      message.channel.send(
        `**${user.tag}** adlı kullanıcı sunucudan yasaklandı.`
      );
      ...
      let embed = new Discord.MessageEmbed()
        .setColor(0xffa300)
        .setAuthor(
          `${user.username} adlı kişi yasaklandı!`,
      ...
      message.channel.send("Yasaklama Başarılı :white_check_mark:");
    }
...
exports.help = {
  name: "ban",
  description: "Kullanıcıı Yasaklar",
  usage: "ban"
};
