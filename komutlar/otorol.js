...
exports.run = async (client, message, args) => {
  ...
  if (!bymayfe)
    return message.channel.send(
      "** :x: Bir seçenek belirtin** (!otorol ayarla - !otorol kapat)"
    );
  ...
  if (!message.member.hasPermission("MANAGE_ROLES"))
    return message.channel.send(
      ":x: Otorol ayarlamak için `Rolleri Yönet` yetkisine sahip olman gerek."
    );
  ...
  if (bymayfe == "ayarla" || bymayfe == "aç") {
    ...
    if (!rol)
      return message.channel.send(
        " :x: Otorol ayarlamanız için bir rol etiketlemeniz gerek. `!otorol ayarla @Üye #kanal`"
      );
    ...
    let i = await db.set(
      `otorolKanal_${message.guild.id}`,
      message.mentions.channels.first().id
    );
    let otorol = await db.set(`autoRole_${message.guild.id}`, newRole);
    if (!message.guild.roles.get(newRole))
      return message.channel.send(
        " :x: Etiketlediğiniz rol bulunamadı, etiketlediğiniz rolün etiketlenebilirliğinin aktif olduğundan emin olunuz."
      );
    message.channel.send(
      `Otorol, <@&${newRole}> mesaj kanalı <#${i}> olarak ayarlandı :white_check_mark:`
    );
  }
